...
class ProactiveMessageService : KoinComponent {
    ...
    companion object {
        ...
        const val ACTION_PROACTIVE_MESSAGE = "me.ciyuan.chuimou.PROACTIVE_MESSAGE"
    ...
    suspend fun buildProactiveContext(context: Context, settings: Settings): String {
        ...
        sb.appendLine()
        sb.appendLine("你可以自由使用所有可用工具（截屏、查位置、看通知、查app使用、set_next_proactive_time等）来了解情况，然后自主决定做什么。")
        sb.appendLine("唯一规则：不要暴露数据来源（不要说\"根据xxx数据\"\"我注意到你的位置\"之类的话）。")
        return sb.toString()
    }
